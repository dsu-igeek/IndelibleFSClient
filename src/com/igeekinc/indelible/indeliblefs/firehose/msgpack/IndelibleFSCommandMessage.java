...
@Message
public abstract class IndelibleFSCommandMessage<V /* Type of the value returned to the caller */, 
	...
	R /* Type of the result from the server */> extends CommandMessage implements AsyncCompletion<R, Void>
{
	...
	public IndelibleFSCommandMessage()
	{
		// for message pack
	}
	
	public IndelibleFSCommandMessage(AsyncCompletion<V, A>callerCompletionHandler, A attachment)
	{
		this.callerCompletionHandler = callerCompletionHandler;
		this.callerAttachment = attachment;
	}

	public IndelibleFSServerCommand getCommand()
	{
		return IndelibleFSServerCommand.getCommandForNum(getCommandCode());
	}
	 ...
	 * implementation
	 */
	public void executeAsync(IndelibleFSFirehoseServerIF server, IndelibleFSClientInfoIF clientInfo, 
			AsyncCompletion<CommandResult, CommandToProcess>completionHandler, CommandToProcess attachment)
	{
		...
		{
			Logger.getLogger(getClass()).debug("Got an exception for command "+getCommandCode());
	 ...
	 * @throws Exception
	 */
	public abstract CommandResult execute(IndelibleFSFirehoseServerIF server, IndelibleFSClientInfoIF clientInfo) throws Exception;

	@Override
	protected int getInitCommandCode()
	{
		return getInitServerCommand().getCommandNum();
	}

	protected abstract IndelibleFSServerCommand getInitServerCommand();
}
