...
@Message
public class NextCASServerEventListItemsReply
{
	public static int kMaxReturnEvents = CASServerEventIteratorReply.kMaxReturnEvents;
	public CASServerEventMsgPack [] events;
	public boolean hasMore;			// Has more after what's been returned in this message
	

	public NextCASServerEventListItemsReply()
	{
		// for message pack
	}
	
	public NextCASServerEventListItemsReply(IndelibleEvent [] versions, boolean hasMore) throws IOException
	{
		this.events = new CASServerEventMsgPack[versions.length];
		for (int eventNum = 0; eventNum < versions.length; eventNum++)
		{
			this.events[eventNum] = new CASServerEventMsgPack((CASServerEvent) versions[eventNum]);
		}
