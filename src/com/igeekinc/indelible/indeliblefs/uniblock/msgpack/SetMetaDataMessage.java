...
@Message
public class SetMetaDataMessage<A> extends CASServerConnectionCommand<Void, A, Void>
{
	public String name;
	public NetworkDataDescriptorMsgPack metadata;
	public SetMetaDataMessage()
	{
		// for message pack
	}
	public SetMetaDataMessage(CASServerFirehoseClient client, CASServerConnectionProxy connection,
			String name, NetworkDataDescriptor metadata,
			AsyncCompletion<Void, A> callerCompletionHandler, A attachment)
	{
		super(client, connection, callerCompletionHandler, attachment);
		this.name = name;
		this.metadata = new NetworkDataDescriptorMsgPack(metadata);
	}
	@Override
	...
	{
		return server.storeMetaData(clientInfo, getConnectionHandle(), name, metadata.getNetworkDataDescriptor());
	}
	@Override
	protected CASServerCommand getInitCASServerCommand()
	{
		return CASServerCommand.kSetMetaData;
	}
}
