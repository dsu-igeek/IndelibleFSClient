...
@Message
public class GetLastReplicatedEventIDMessage<A> extends CASServerConnectionCommand<Long, A, Long>
{
	public ObjectIDMsgPack sourceServerID;
	public ObjectIDMsgPack collectionID;
	
	public GetLastReplicatedEventIDMessage()
	{
		// for message pack
	}

	public GetLastReplicatedEventIDMessage(CASServerFirehoseClient client, CASServerConnectionProxy connection,
			EntityID sourceServerID, CASCollectionID collectionID, AsyncCompletion<Long, A> callerCompletionHandler, A attachment)
	{
		super(client, connection, callerCompletionHandler, attachment);
		this.sourceServerID = new ObjectIDMsgPack(sourceServerID);
	...
	{
		return server.getLastReplicatedEventID(clientInfo, getConnectionHandle(), (EntityID)sourceServerID.getObjectID(),
				(CASCollectionID)collectionID.getObjectID());
	}

	@Override
	protected CASServerCommand getInitCASServerCommand()
	{
		return CASServerCommand.kGetLastReplicatedEventID;
	}
}
